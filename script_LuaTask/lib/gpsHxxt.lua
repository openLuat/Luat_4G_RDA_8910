...
--- 设置GPS模块搜星模式.
--[[
    H01(1) –GPS L1+SBAS+QZSS
    H10 – BDS B1
    H101 2 – GPS+GLONASS+GALILEO+SBAS+QZSS
    H11 3 – GPS+BDS+GALILEO+SBAS+QZSS
]]
...
function setNemaReportFreq(date,flag)
    ...
    if date=="RMC" then
        ...
        else if date=="GGA" then
            writeCmd("$CFGMSG,0,0,"..flag)
        else if date=="GSA" then
            writeCmd("$CFGMSG,0,2,"..flag)
        else if date=="VTG" then
            writeCmd("$CFGMSG,0,5,"..flag)
        else if date=="GLL" then
            writeCmd("$CFGMSG,0,1,"..flag)
        else if date=="GSV" then
